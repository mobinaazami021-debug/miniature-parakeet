...
function randomTriangle(i) {
    points[i] = [
        {x: randBetween(50,290), y: randBetween(50,290)},
        {x: randBetween(50,290), y: randBetween(50,290)},
        {x: randBetween(50,290), y: randBetween(50,290)}
    ];
    draw(i);
...
function checkCongruent(){
    ...
    let ok = (Math.abs(s1[0]-s2[0]) < 12 &&
              Math.abs(s1[1]-s2[1]) < 12 &&
              Math.abs(s1[2]-s2[2]) < 12);

    if (ok) {
        document.getElementById("farz").innerText = "سه ضلع برابرند.";
        document.getElementById("hokm").innerText = "دو مثلث هم‌نهشت‌اند.";
        document.getElementById("proofBox").innerText =
            "چون (ض.ض.ض) برقرار است، پس مثلث‌ها هم‌نهشت‌اند.";
        addScore();
    } else {
        document.getElementById("proofBox").innerText = "هم‌نهشت نیستند.";
    }
}
